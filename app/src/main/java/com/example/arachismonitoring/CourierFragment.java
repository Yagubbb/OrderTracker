...
public class CourierFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // Inflate the menu with the filter icon for the courier section
    ...
    public void fetchSheetData() {
        ...
        System.out.println("PAYMENT_DEBUG: fetchSheetData called");
        ...
        new Thread(() -> {
            try {
                ...
                // Verify if the user has access to this sheet
                try {
                    ...
                    if (values != null && values.size() > 1) {
                        for (int i = 1; i < values.size(); i++) {
                            ...
                            if (row.size() > 12) {
                                System.out.println("PAYMENT_DEBUG Row " + i + " column M: " + row.get(12));
                            } else {
                                System.out.println("PAYMENT_DEBUG Row " + i + " column M: <not present>");
                            }
                    ...
                    requireActivity().runOnUiThread(() -> {
                        Toast.makeText(getContext(), "You don't have access to this sheet", Toast.LENGTH_LONG).show();
                        swipeRefresh.setRefreshing(false);
                    });
                }
            } catch (Exception e) {
